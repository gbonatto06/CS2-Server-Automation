...
MONITOR EVERYTHING VIA GRAFANA:
  Direct Link: http://${aws_instance.cs2_server.public_ip}:3000/d/cs2_01/servidor-de-cs2?kiosk=1
...
ACCESS & DEBUGGING:
  - S3 Bucket (Database): https://s3.console.aws.amazon.com/s3/buckets/${local.bucket_name}?region=${var.aws_region}
  - SSH Access: ssh -i cs2-server-key.pem ubuntu@${aws_instance.cs2_server.public_ip}
  - Manual Debug: ssh -i cs2-server-key.pem ubuntu@${aws_instance.cs2_server.public_ip} "tail -f /var/log/user-data.log"
